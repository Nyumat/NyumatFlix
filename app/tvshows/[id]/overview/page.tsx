...
import { Creator, Genre } from "@/utils/typings";
...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  return { ...base, title: `Overview · ${nameWithYear} | NyumatFlix` };
}

export default async function TVShowOverviewPage(props: Props) {
  const params = await props.params;
  const { id } = params;
        ...
        <span className="flex flex-wrap gap-x-1">
          {networks.map((n: { id: number; name: string }, i: number) => (
            <span key={n.id}>
              {i > 0 ? ", " : null}
              <Link
                href={`${pages.search.link}?q=${encodeURIComponent(n.name)}`}
                className={factLinkClass}
              >
                {n.name}
              </Link>
