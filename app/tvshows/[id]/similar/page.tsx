...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  if (!m || !("name" in m)) return { title: "Similar" };
  return { title: `Similar · ${m.name}` };
}

export default async function TVShowSimilarPage(props: Props) {
  const { id } = await props.params;
