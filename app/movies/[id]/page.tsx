...
import { MovieCollection } from "@/components/movie/movie-server";
...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  const movie = await getCachedMovieDetail(params.id);
...
export default async function MovieOverviewPage(props: Props) {
  const params = await props.params;
  const raw = await getCachedMovieDetail(params.id);
  if (!raw || !("title" in raw)) {
    notFound();
  }
  const details = raw as unknown as MovieDetails;
          ...
          {production_companies.map(
            (c: { id: number; name: string }, i: number) => (
              <span key={c.id}>
                {i > 0 ? ", " : null}
                <Link
                  href={`${pages.search.link}?q=${encodeURIComponent(c.name)}`}
