...
import { compareStreamRank, isStereoscopicRelease, streamPickScore, } from "./streamRank.js";
...
export function rankDirectStreams(streams) {
    return streams
        ...
        .sort((left, right) => directStreamPickScore(right) - directStreamPickScore(left));
}
...
export function rankDirectMovieStreamsForPlayback(streams) {
    const twoDimensional = streams.filter((stream) => !isStereoscopicDirectStream(stream));
