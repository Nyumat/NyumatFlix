...
function streamBlob(stream: PlaybackHintInput): string {
...
function hasDvheProfile(blob: string, profile: number): boolean {
  const n = String(profile);
  return new RegExp(String.raw`\bdv(?:he|av|h1)\.?0?${n}(?:\.|\b)`, "i").test(
    blob,
  );
}

function hasDolbyVisionTag(blob: string): boolean {
  return (
    /\b(?:dovi|dolby[.\-_ ]?vision)\b/.test(blob) ||
    /\bdvhe\b/.test(blob) ||
    /\bdv\b/.test(blob)
  );
}

function hasHdr10CompatibleFallback(blob: string): boolean {
  return (
    /\bhdr10(?:\+|plus)?(?:[.\-_ ]compatible)?\b/.test(blob) ||
    /\bhdr\b/.test(blob) ||
    /\bhlg\b/.test(blob) ||
    /\bhybrid\b/.test(blob)
  );
...
export function isIncompatibleDolbyVision(stream: PlaybackHintInput): boolean {
...
export function shouldSkipBrowserPlayback(stream: PlaybackHintInput): boolean {
  return (
    isTooHeavyForBrowserPlayback(stream) || isIncompatibleDolbyVision(stream)
  );
}

export function streamSizeBytes(stream: PlaybackHintInput): number {
  const raw = stream.size;
  if (typeof raw === "number" && raw > 0) return raw;
  if (typeof raw === "string") {
    const trimmed = raw.trim();
    const gb = trimmed.match(/^([\d.]+)\s*gi?b$/i);
    if (gb) return Math.round(Number(gb[1]) * 1e9);
    const mb = trimmed.match(/^([\d.]+)\s*mi?b$/i);
    if (mb) return Math.round(Number(mb[1]) * 1e6);
...
export function isRemuxRelease(stream: PlaybackHintInput): boolean {
  return /remux|complete\.bluray/i.test(streamBlob(stream));
}

const MAX_BROWSER_PLAYBACK_GB = 100;

export function isTooHeavyForBrowserPlayback(
...
export function hasHeavyCodecTags(stream: PlaybackHintInput): boolean {
  return /\b(hevc|x265|h\.?265|av1|dts|truehd|atmos|ma\.?\s*5\.?1)\b/.test(
    streamBlob(stream),
  );
}

export function is4KRelease(stream: PlaybackHintInput): boolean {
  const blob = streamBlob(stream);
  return /\b(2160p|4k|uhd)\b/.test(blob);
}

export function inferTranscodeProfile(
  stream: PlaybackHintInput,
): TranscodeProfile {
  const bytes = streamSizeBytes(stream);
  const gb = bytes / 1e9;
...
export function inferPlaybackEngineHint(
  stream: PlaybackHintInput,
): PlaybackEngineHint {
...
export function isBrowserPlayableRelease(name: string, mime?: string): boolean {
  if (requiresExtendedPlayback(name)) return false;
  if (/\.(mp4|m4v|webm)(?:[?#]|$)/i.test(name)) return true;
 ...
 */
export function isBrowserStreamCandidate(
...
export function filterBrowserStreams<T extends BrowserStreamCandidateInput>(
  streams: readonly T[],
): T[] {
  return streams.filter(isBrowserStreamCandidate);
}

export function partitionBrowserStreams<T extends BrowserStreamCandidateInput>(
  streams: readonly T[],
): { browser: T[]; external: T[] } {
  const browser: T[] = [];
  const external: T[] = [];
...
export function prefersTranscodeFirst(stream: PlaybackHintInput): boolean {
...
export function prefersProgressiveTranscode(
  stream: PlaybackHintInput,
): boolean {
  if (!isRemuxRelease(stream)) return false;
  return /\.mp4(?:[?#]|$)/i.test(streamBlob(stream));
}

export function isProgressiveTranscodePath(path: string): boolean {
  return path.includes("/api/transcode/stream");
}

/** HLS playlist fallback from MediaFlow → matching progressive fMP4 transcode URL. */
export function companionProgressiveTranscodePath(
  transcodePath: string,
): string | null {
  if (!transcodePath.includes("/api/transcode/playlist")) {
    return null;
  }
  return transcodePath.replace(
    "/api/transcode/playlist",
