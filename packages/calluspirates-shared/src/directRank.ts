...
import {
  ...
  streamPickScore,
  type StreamRankInput,
} from "./streamRank.js";

export type DirectStreamRankInput = StreamRankInput;

export function isStereoscopicDirectStream(
  stream: DirectStreamRankInput,
): boolean {
  return isStereoscopicRelease(`${stream.fileName ?? ""} ${stream.name}`);
}

export function directStreamPickScore(stream: DirectStreamRankInput): number {
...
export function rankDirectStreams<T extends DirectStreamRankInput>(
  streams: T[],
): T[] {
  return streams
...
export function pickBestDirectStream<T extends DirectStreamRankInput>(
  streams: T[],
): T | null {
  return rankDirectStreams(streams)[0] ?? null;
}

function playbackTier(stream: DirectStreamRankInput): number {
...
function streamByteSize(
  stream: DirectStreamRankInput & { size?: number | string },
): number {
...
export function rankDirectMovieStreamsForPlayback<
  T extends DirectStreamRankInput & {
    cached?: boolean;
    size?: number | string;
  },
>(streams: T[]): T[] {
...
export function rankDirectMovieStreams<T extends DirectStreamRankInput>(
  streams: T[],
): T[] {
  return rankDirectMovieStreamsForPlayback(streams);
}

export function pickBestDirectMovieStream<T extends DirectStreamRankInput>(
  streams: T[],
): T | null {
