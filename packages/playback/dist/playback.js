...
import { companionProgressiveTranscodePath, isProgressiveTranscodePath, isTooHeavyForBrowserPlayback, } from "@calluspirates/shared";
import { pickTrackIndexByLanguage, trackMatchesLanguage, isJapaneseAudioPreference, } from "./track-matching";
function streamNameBlob(stream) {
    return `${stream.fileName ?? ""} ${stream.name}`.toLowerCase();
}
export const looksLikeHeavyMoviRemux = (stream) => /remux|complete\.bluray/i.test(streamNameBlob(stream));
const isHeavyBrowserDecode = (stream) => {
    if (looksLikeHeavyMoviRemux(stream)) {
        return true;
    }
    if (stream.transcodeProfile === "heavy" ||
...
function canUseMoviEngine(stream) {
    if (!isHeavyBrowserDecode(stream)) {
        return true;
    }
    return !stream.fallbackUrl;
}
export const pickEnglishMoviLang = (tracks) => {
    const scored = tracks
        .map((track) => {
...
export const pickMoviTrackPrefs = (audioTracks, subtitleTracks, preferredAudioLang) => {
    if (preferredAudioLang) {
        const preferredIndex = pickTrackIndexByLanguage(audioTracks, preferredAudioLang);
        const audioLang = preferredIndex != null
            ? (audioTracks[preferredIndex]?.lang ?? null)
            : pickEnglishMoviLang(audioTracks);
        const audioIsEnglish = audioLang != null && trackMatchesLanguage({ lang: audioLang }, "english");
        const wantEnglishSubs = isJapaneseAudioPreference(preferredAudioLang) || !audioIsEnglish;
        return {
            audioLang,
...
const isBrowserDirectMp4 = (stream) => stream.playback === "direct" && stream.browserPlayable === true;
export const orderDirectStreamsForScrape = (ranked, preferMultiTrack) => {
    ...
    const bySize = (left, right) => (left.size && left.size > 0 ? left.size : Number.MAX_SAFE_INTEGER) -
    ...
    const rest = extendedOrdered.filter((stream) => !looksLikeMultiTrackStream(stream));
    if (preferMultiTrack) {
        return [...multi, ...rest, ...directMp4];
    }
    return [...directMp4, ...extendedOrdered];
};
export function looksLikeMultiTrackStream(stream) {
    const blob = streamNameBlob(stream);
    return (/\bmulti(?:ple)?(?:[.\-_ ]?(?:audio|audios|sub|subs|subtitle|subtitles|lang|langs|language|languages))?\b/.test(blob) ||
        /\bdual[.\-_ ]?(?:audio|audios|sub|subs|subtitle|subtitles|lang|langs)\b/.test(blob) ||
        /\b(?:ja|jpn|jp|jap|japanese)[.\-_ +/&]+(?:en|eng|english)\b/.test(blob) ||
        /\b(?:en|eng|english)[.\-_ +/&]+(?:ja|jpn|jp|jap|japanese)\b/.test(blob));
}
function looksLikeMkvStream(stream) {
    return /\.mkv(?:[?#]|$)/i.test(streamNameBlob(stream));
}
export function shouldRetainMoviEngine(stream) {
    if (isHeavyBrowserDecode(stream)) {
        return false;
    }
...
export function prefersMoviFirstEngine(stream) {
    if (isHeavyBrowserDecode(stream)) {
        return false;
    }
    if (stream.playbackHint === "movi-first") {
...
export function isFastStartDirectStream(stream) {
    return Boolean(stream?.browserPlayable && stream.playback === "direct");
}
function addVidstackTranscodeCandidates(stream, add) {
...
function toRoutingTranscodePath(path) {
    return path.replace("/api/direct/transcode/", "/api/transcode/");
}
function isClientProgressiveTranscodePath(path) {
    return isProgressiveTranscodePath(toRoutingTranscodePath(path));
}
export function isDirectProgressiveTranscodePath(path) {
    return isClientProgressiveTranscodePath(path);
}
function clientCompanionProgressiveTranscodePath(transcodePath) {
    return companionProgressiveTranscodePath(toRoutingTranscodePath(transcodePath));
}
const engineLabel = (engine, sourceUrl) => {
    if (engine === "movi")
...
export function engineSourceUrl(stream, engine) {
    ...
    if (engine === "vidstack-direct" && stream.fallbackUrl) {
        ...
        const progressive = clientCompanionProgressiveTranscodePath(stream.fallbackUrl);
        if (progressive) {
            return progressive;
        }
    }
...
export function engineStreamKind(engine, sourceUrl) {
    if (engine === "vidstack-hls") {
        return "hls";
    }
...
export function openDownloadUrl(stream) {
