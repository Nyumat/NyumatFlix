...
export function WatchlistGridFallback() {
  return (
    ...
    >
      ...
      <div className="rounded-lg border border-white/10 bg-background/82 p-4 shadow-xl shadow-black/20 backdrop-blur-xl md:p-5">
      ...
      <div className="rounded-lg border border-white/10 bg-card/28 p-3 shadow-lg shadow-black/10 backdrop-blur-md">
      ...
      <div className="space-y-4">
        {["watching", "plan_to_watch", "on_hold", "dropped", "completed"].map(
          (section) => (
            <div
              key={section}
              className="rounded-lg border border-white/10 bg-card/20 p-4 shadow-lg shadow-black/10"
            >
              ...
              <div className="grid gap-4 [grid-template-columns:repeat(auto-fill,minmax(min(100%,320px),1fr))]">
                {Array.from({ length: 2 }).map((_, index) => (
                  <div
                    key={index}
                    className="relative min-h-44 overflow-hidden rounded-lg border border-white/12 bg-black/30 shadow-lg shadow-black/20"
                  >
                    ...
                    <div className="relative flex min-h-44 flex-col justify-between p-4">
                      <div className="max-w-[70%] space-y-2 rounded-md border border-white/10 bg-black/25 px-3 py-2">
